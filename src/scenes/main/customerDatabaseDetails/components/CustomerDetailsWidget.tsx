...
import type { TBookingCustomerDetails } from "@/api/entities/user/customer";
...
const CustomerDetailsWidget = ({
  ...
  customer?: TBookingCustomerDetails;
  bowersUsage?: Date;
}) => {
  const getName = () => {
  ...
  return (
    <div className="w-full p-5 rounded-xl bg-darkPrimary">
      <div className="w-full pb-6 flex items-start justify-between border-b border-greyOutlineSecondary">
        <h3 className="text-xl font-bold text-white">{getName()}</h3>
      </div>
      <div className="pt-6 flex flex-col gap-2">
        ...
        <Row
          label="Bowers Usage"
          value={bowersUsage ? format(bowersUsage, "dd MMM yyyy") : ""}
        />
      </div>
    </div>
  );
